...
class IntegrityError(RuntimeError):
    """Raised when a GJ 486 b input differs from the frozen manifest."""
...
def verify_archive(path: Path, spec: ArchiveSpec) -> dict[str, object]:
    ...
    if md5 != spec.publisher_md5:
        raise IntegrityError(f"archive MD5 mismatch: expected {spec.publisher_md5}, got {md5}")
...
def verify_member(path: Path, spec: MemberSpec) -> dict[str, object]:
    ...
    if size != spec.size_bytes:
        raise IntegrityError(f"member {spec.path} size mismatch: expected {spec.size_bytes}, got {size}")
    sha256 = sha256_file(path)
    if sha256 != spec.sha256:
        raise IntegrityError(f"member {spec.path} SHA-256 mismatch: expected {spec.sha256}, got {sha256}")
