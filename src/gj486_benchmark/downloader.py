...
def download_archive(
    ...
    *,
    ...
    if source == "nirspec":
        ...
        spec = manifest.external_constraint.archive
    else:
        raise DownloadError(f"unknown source {source!r}; expected 'nirspec' or 'miri'")
    ...
    try:
        ...
        if md5.hexdigest() != spec.publisher_md5:
            raise DownloadError(f"MD5 mismatch: expected {spec.publisher_md5}, got {md5.hexdigest()}")
        if sha256.hexdigest() != spec.sha256:
            raise DownloadError(f"SHA-256 mismatch: expected {spec.sha256}, got {sha256.hexdigest()}")
